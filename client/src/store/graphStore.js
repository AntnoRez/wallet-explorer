...
export const useGraphStore = create((set, get) => ({
  ...
  setFilters: (patch) => {
    ...
    // Закрепления и кэш переживают смену фильтра: это настройка показа,
    // а не новое расследование. Пересобранный граф подтянет закреплённые
    // узлы из кэша, даже если в новую выборку они не попали
  ...
  load: async (address, { reset = true, refresh = false, loadMore = false, newChain = false } = {}) => {
    ...
    try {
      // Сколько контрагентов показывать — решает фильтр в шапке, и он же
      // решает это при раскрытии.
      //
      // Раньше при раскрытии подставлялось фиксированное число, меньшее
      // фильтра: соседи ведь накапливаются с каждым шагом. Но тогда
      // подпись в шапке врала — обещала двадцать, показывала десять,
      // а переключатель «Узлов» переставал что-либо менять. Хочешь
      // меньше на шаг — выбери меньше, это честнее.
